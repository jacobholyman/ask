...
출력 형식:
  ask "질문"                 ✨ 기본이 마크다운 렌더링 (alias로 -M 자동 적용)
  \ask "질문"                raw 스트리밍 (alias 우회, 백슬래시 prefix)
  ask -M "질문"              명시적 마크다운 (alias 없어도 동작)
  (모든 응답 끝에 ⏱ 시간 표시)
...
`~/.bashrc`에 `alias ask='ask -M'` 등록되어 있어 **기본이 마크다운 모드**입니다.

```bash
ask "Python 데코레이터 예제 3개를 표로 정리"   # 자동 마크다운
\ask "raw 스트리밍 보고 싶을 때"              # alias 우회
```
...
**alias 적용된 ask vs `\ask` 비교**:
| 명령 | 응답 표시 | 장점 | 단점 |
|---|---|---|---|
| **`ask`** (alias=`ask -M`) | 끝에 rich 렌더링 | 깔끔, 코드 하이라이트 | 응답 끝나야 보임 |
| `\ask` (alias 우회) | raw 스트리밍 | 즉시 피드백 | 마크다운 raw (`**`, `#`) |
